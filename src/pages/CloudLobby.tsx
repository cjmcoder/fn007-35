...
export default function CloudLobby() {
  ...
  const { match, roomSize, status, error, cancelSearch } = useLobbySocket('cloud', matchId);

  return (
    <Shell mode="cloud" matchId={matchId}>
    ...
    <div className="min-h-screen bg-black text-white p-6">
      <div className="max-w-2xl mx-auto space-y-4">
        <header className="flex items-center gap-3">
          <span className="px-3 py-1 bg-emerald-700 text-xs rounded-full">Cloud Stream 1v1</span>
...
function State({ status, error, roomSize, match, matchId, onCancel }: any) {
  if (status === 'connecting') return <Card title="Connecting…" subtitle="Preparing your cloud session…" />;
  ...
  if (status === 'active' && match) return (
    <Card title="Opponent found!" subtitle={`Entry Fee: ${match.entryFee} FC`}>
      <a href={`/matches/${match.id}`} className="rounded-xl px-4 py-3 bg-emerald-600 hover:bg-emerald-500 inline-block">Go to Match</a>
    </Card>
  );
