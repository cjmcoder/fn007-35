...
import { CreditCard, Plus, Trash2, Download, Shield, Wallet, DollarSign } from 'lucide-react';
...
export function ProfilePayments() {
  ...
  const [boostCredits] = useState(275);
  const [fncBalance] = useState(12450);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="gaming-card border-primary/30">
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">Withdrawable Credits</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-primary mb-2">{fcBalance.toLocaleString()} FC</div>
            <div className="flex space-x-2">
              <Button size="sm" onClick={handleTopUp} className="bg-gradient-primary flex-1">
        ...
        {/* Boost Credits */}
        <Card className="gaming-card border-neon-orange/30">
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center space-x-2 text-base">
              <Wallet className="h-5 w-5 text-neon-orange" />
              <span>Boost Credits</span>
            </CardTitle>
            <CardDescription className="text-xs">Play-Only Bonus</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-neon-orange mb-2">{boostCredits.toLocaleString()} BC</div>
            <div className="text-xs text-muted-foreground">
              Non-withdrawable promotional credits
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="gaming-card border-accent/30">
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">Performance Rewards</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-accent mb-2">{fncBalance.toLocaleString()} FNC</div>
            <div className="text-xs text-muted-foreground">
      ...
      {/* Balance Notice */}
      {fcBalance < 100 && (
        <Card className="gaming-card bg-muted/20 border-border">
          <CardContent className="pt-6">
            <div className="text-sm text-muted-foreground">
              <strong>Withdrawal Notice:</strong> Minimum withdrawal amount is 100 FC. Withdrawals are subject to review and may take 3-5 business days.
            </div>
          </CardContent>
        </Card>
      )}
